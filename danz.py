...
⠀⢧⠀⠀⠀⠀⠹⢤⡀⠀⠀⠀⣻⡆⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⡴⢲⠇⠀⠀⠀⠀⠀⠞⠀⠀⠀⣸⠀
⠀⠈⠳⠤⣀⣀⣀⣀⡤⠤⠤⠤⠵⠃⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠻⠯⢤⣀⣀⣀⡤⠤⠤⠤⠤⠴⠏⠀"""
    utils.print(f'{C}{art}{W}\n')
    utils.print(f'{W}[>] {R}Created By   : {W}Danz')
    utils.print(f'{W}[>] {R}Version      : {W}{VERSION}\n')
...
def template_select(site):
    ...
    print()
    utils.print(
        f'{W}[+] {G}Loading {Y}{templ_json["templates"][selected]["name"]} {C}Template...{W}'
    )
...
def server():
    print()
    port_free = False
    utils.print(f'{Y}[+] {C}Port : {W}{port}\n')
    utils.print(f'{Y}[+] {C}Starting PHP Server...{W}', end='')
